...
def page_view_received(sender, **kwargs):
    ...
    if not user.is_authenticated():
        ...
        new_page_view = PageView.objects.create(user=user, path=page_path, timestamp=timezone.now())

    # if sender.is_anonymous:
    #     new_page_view, created = PageView.objects.get_or_create(path=page_path, timestamp=timezone.now())
    # else:
    #     new_page_view, created = PageView.objects.get_or_create(user=user, path=page_path, timestamp=timezone.now())
    # if not created:
    #     new_page_view.count += 1  # 1 by default if created
    #     new_page_view.save()
